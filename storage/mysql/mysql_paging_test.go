...
func TestPagination(t *testing.T) {
	...
	for {
		if pagingType == "id" {
			...
			rows, err = mysql.Select("SELECT id, name, login_id FROM users WHERE id > ? ORDER BY id ASC LIMIT ?", lastestId, getCount)
		} else {
			rows, err = mysql.Select("SELECT id, name, login_id FROM users LIMIT ?, ?", (pageNum-1)*getCount, getCount)
		}
		if err != nil {
			log.Fatal(err)
		}
